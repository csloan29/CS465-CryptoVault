...
class AESCipher:
    ...
    def cipher(self, data_in, key):
        # Verbose Details
        print("\nCIPHER  (ENCRYPT):")
        print("round[ 0].input   ", data_in)
        ...
        # Verbose Details
        print("round[ {}].k_sch    {}".format(0, return_key_array_to_text(round_key, self.NB)))

        # round tracker for expanded key needed for particular round of encryption
        cipher_round = 1

        # iterate through rounds of encryption
        while cipher_round is not self.NR:
            print("round[ {}].start    {}".format(cipher_round, return_state_array_to_text(state, self.NB)))
            state = self.sub_bytes(state)
            print("round[ {}].s_box    {}".format(cipher_round, return_state_array_to_text(state, self.NB)))
            state = self.shift_rows(state)
            print("round[ {}].s_row    {}".format(cipher_round, return_state_array_to_text(state, self.NB)))
            state = self.mix_columns(state)
            print("round[ {}].m_col    {}".format(cipher_round, return_state_array_to_text(state, self.NB)))
            round_key = expanded_key[:, (cipher_round * self.NB):((cipher_round + 1) * self.NB)]
            state = self.add_round_key(state, round_key)
            print("round[ {}].k_sch    {}".format(cipher_round, return_key_array_to_text(round_key, self.NB)))
            cipher_round += 1
        ...
        state = self.add_round_key(state, round_key)
        print("round[ {}].k_sch    {}".format(cipher_round, return_state_array_to_text(state, self.NB)))

        # send encrypted data out
        print("round[ {}].output   {}".format(cipher_round, return_state_array_to_text(state, self.NB)))
    ...
    def sub_bytes(self, state):
        for x in range(0, self.NB):
            for y in range(0, self.NB):
                ...
                state[x][y] = s_box[first_nibble][second_nibble]
        return state
    ...
    def shift_rows(self, state):
        ...
        for x in range(1, self.NB):
            ...
            for y in range(0, self.NB):
                new_row[y] = state[x][(y + increment) % self.NB]
            state[x] = new_row
            increment += 1
        return state
            ...
            for x in range(0, self.NB):
                ...
                while count < self.NB:
                    new_arr[x][y] = ff_add(new_arr[x][y], ff_multiply(
                        state[count][y], static_mix_arr[x][count]))
                    count += 1
        return new_arr
